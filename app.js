    ...
    Users.create({
      ...
      .catch((error) => {
        ...
        return res.render("error", {
          error,
          text: "Some error occured, while adding! Please try again.",
        });
      });
  } else {
    return res.render("error", {
      error: "Please enter all details to signup!",
    });
  }
});
    ...
    Users.findOne({
      ...
      .then((user) => {
        if (!user) {
          return res.render("error", {
            error:
              "Account does not exist! Please try logging in again with correct email.",
          });
          ...
          return res.render("error", {
            error:
              "Invalid E-mail or Password! Please try logging in again with correct details.",
          });
        }
      })
      .catch((error) => {
        ...
        return res.render("error", {
          error,
          text: "Some error occurred! Please try logging in again.",
        });
      });
  } else {
...
app.post("/admin_login", (req, res) => {
  if (req.body.email && req.body.pass) {
    ...
    // })
    if (
      req.body.email == "devrev@gmail.com" &&
      req.body.pass == "wewillcrackit"
    ) {
...
app.post("/add_hospital", (req, res) => {
  ...
  if (
    ...
    return res.render("error", {
      error: "Please enter all details to add a hospital!",
    });
  }
});
...
app.post("/add_question", (req, res) => {
  ...
  if (req.body.email && req.body.question) {
    ...
    return res.render("error", {
      error: "Please enter all details to add a question!",
    });
  }
});
...
app.post("/update_bed_cnt", (req, res) => {
  if (req.body.bed_cnt && req.body.id) {
    ...
    return res.render("error", {
      error: "Please enter all details to update bed count!",
    });
  }
});
...
app.post("/getslots", (req, res) => {
  if (req.body.hosp_id && req.body.apt_date) {
    ...
    return res.render("error", {
      error: "Please enter all details to find slots!",
    });
  }
});
...
app.post("/book_vac_appt", (req, res) => {
  if (
      ...
      .then((apt) => {
        ...
        Hospital_Slots.findOne({
          ...
          .then((hosp_slot) => {
            ...
            new_string =
              slot_string.substring(0, req.body.slot - 1) +
              "1" +
              slot_string.substring(req.body.slot);
            Hospital_Slots.update(
              { slots: new_string },
              {
                where: {
                  hospital_id: req.body.hospital_id,
    ...
    return res.render("error", {
      error: "Please enter all details to book vaccination appointment!",
    });
  }
});
...
app.post("/req_bed", (req, res) => {
  if (req.body.email && req.body.symptoms && req.body.hospital_id) {
    ...
    return res.render("error", {
      error: "Please enter all details to request a bed!",
    });
  }
});
...
app.post("/complete_vac", (req, res) => {
  if (req.body.email) {
    ...
    return res.render("error", {
      error: "Please enter all details to update appt!",
    });
  }
});
...
app.post("/complete_bed", (req, res) => {
  if (
    ...
    Bed_Reqs.update(
      { status: "Approved", instructions: req.body.instructions },
      {
        where: {
          id: req.body.id,
        },
    ...
    return res.render("error", {
      error: "Please enter all details to approve bed req!",
    });
  }
});
...
app.post("/deny_bed", (req, res) => {
  if (req.body.email && req.body.instructions && req.body.id) {
    Bed_Reqs.update(
      { status: "Denied", instructions: req.body.instructions },
      {
        where: {
          id: req.body.id,
        },
    ...
    return res.render("error", {
      error: "Please enter all details to deny bed req!",
    });
  }
});
