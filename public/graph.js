...
const populateDropdown = (countries) => {
  const country_options = countries
    .map((str) => `<option value="${str}">${str}</option>`)
    .join("");
};
...
const updateTotals = (countries) => {
  ...
  const maxs = Object.entries(totalMap).map(([k, v]) => [
    k,
    Math.max(...countries.map((o) => +o[v] || 0)),
  ]);
...
const init = async (country) => {
  ...
  document.getElementById("last-updated").innerText = getUKDateFormat(
    country_data[country_data.length - 1].date
  );
};
